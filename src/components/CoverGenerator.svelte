...
// Font selection
let fonts = [
	{ name: 'sans-serif', label: '无衬线字体 (系统默认)' },
...
onMount(async () => {
	...
	try {
		...
		if ('queryLocalFonts' in window) {
			...
			// @ts-ignore: Font Access API is not yet in TypeScript definitions
			const systemFonts = await window.queryLocalFonts();
			// Add system fonts to the list
			const fontFamilies = new Set();
			systemFonts.forEach(font => {
				fontFamilies.add(font.family);
			});
			// Convert to array and add to fonts list
			const uniqueSystemFonts = Array.from(fontFamilies).filter(family => 
				!fonts.some(font => font.name === family)
			);
			uniqueSystemFonts.forEach(family => {
				fonts.push({ name: family, label: family });
			});
		}
	} catch (error) {
...
<div class="flex flex-col items-center gap-8 w-full max-w-6xl mx-auto relative">
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full bg-[var(--card-bg)] p-6 rounded-xl transition-colors duration-300" style="--hue: {hue}">
    ...
    <div class="flex flex-col gap-6">
        ...
        <div class="space-y-6">
            <div class="flex flex-col gap-2">
                <label class="text-sm font-bold text-gray-700 dark:text-gray-300">字体选择</label>
                <select bind:value={selectedFont} class="input-field w-full">
                    {#if isLoadingFonts}
                        <option value="sans-serif">加载系统字体中...</option>
                    {:else}
                        {#each fonts as font}
                ...
                {#if isLoadingFonts}
                    <span class="text-xs text-gray-500">正在获取系统字体，请稍候...</span>
                {:else if !fontApiSupported}
                    <span class="text-xs text-gray-500">当前浏览器不支持获取系统字体，显示的是常用字体</span>
                {:else if fonts.length <= 10}
                    <span class="text-xs text-gray-500">系统字体获取完成</span>
                {/if}
            </div>
            <div class="flex flex-col gap-2">
                <label class="text-sm font-bold text-gray-700 dark:text-gray-300">字重选择</label>
                <select bind:value={selectedFontWeight} class="input-field w-full">
